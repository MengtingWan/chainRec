...
class Dataset(object):
    ...
    def split_train_test(self, method="byUser", seed=None, 
    ...
    def sampling(self, method="edgeOpt_uniform", N_TRAIN=1000000, N_NEG=5, dump_samples=False):
        ...
        print("Start sampling training and validation data ...")
        sys.stdout.flush()
        
        if method == "sliceOpt":
            
            training_samples = np.array([]).reshape(0, N_NEG+3)
            validation_samples = []
            count = 0
            ...
            for u in user_item_map:
                ...
                for sid in range(max_stage):
                    ...
                    if n_pos>0:          
                        ...
                        tmp = np.append(tmp, neg_i_list[:-1].reshape(n_pos, N_NEG), axis=1)
                        training_samples = np.append(training_samples, tmp, axis=0)
                        
                        if sid_vali >= sid:
                            validation_samples.append([u, iid_vali, sid, neg_i_list[-1]])
                    ...
                    if n_pos>0:          
                        ...
                        if sid == 0:
                            ...
                            if len(items_s_neg) > 0:
                                ...
                                neg_i_list = np.random.choice(n_item, size=n_pos*N_NEG+1)
                        tmp = np.append(tmp, neg_i_list[:-1].reshape(n_pos, N_NEG), axis=1)
                        training_samples = np.append(training_samples, tmp, axis=0)
            ...
            training_samples = np.array([]).reshape(0, N_NEG*2+3)
            validation_samples = []
            count = 0
            print("current user: ", end="")
            sys.stdout.flush()       
            for u in user_item_map:
                ...
                for sid in range(max_stage):
                    ...
                    if n_pos>0:          
                        ...
                        tmp_neg = np.array([neg_i_list[:-1], neg_i_stage[:-1]]).transpose()
                        tmp = np.append(tmp, tmp_neg.reshape(n_pos, N_NEG*2), axis=1)
                        training_samples = np.append(training_samples, tmp, axis=0) 
                        
                        if sid_vali >= sid:
                            validation_samples.append([u, iid_vali, sid, neg_i_list[-1], neg_i_stage[-1]])
            validation_samples = np.array(validation_samples, dtype=int)
            print("done!")
            sys.stdout.flush()      
            ...
            for u in user_item_map:
                ...
                for sid in range(max_stage):
                    ...
                    if n_pos>0:          
                        ...
                        p /= np.sum(p)
                        neg_i_list = np.random.choice(n_item, size=n_pos*N_NEG+1, p=p)
                        neg_i_stage = stage_map[neg_i_list]
                        tmp_neg = np.array([neg_i_list[:-1], neg_i_stage[:-1]]).transpose()
                        tmp = np.append(tmp, tmp_neg.reshape(n_pos, N_NEG*2), axis=1)
                        training_samples = np.append(training_samples, tmp, axis=0) 
